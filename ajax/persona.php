		...
		$rspta=$persona->listarp();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
 				...
 				"3"=>$reg->num_documento,
				 "4"=> '<a class="btn btn-warning" href="tel:"'.$reg->telefono.'">'.$reg->telefono. '</a>'.
				 	'<a class="btn btn-danger" href="https://wa.me/521"'.$reg->telefono.'">'.$reg->telefono. '</a>',
 				"5"=>$reg->email
 				);
 		}
 		$results = array(
		...
		$rspta=$persona->listarc();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
 				...
 				"2"=>$reg->tipo_documento,
 				"3"=>$reg->num_documento,
				 "4"=> '<a class="btn btn-info" href="tel:"'.$reg->telefono.'">'.$reg->telefono. '</a>'.
				 	'<a class="btn btn-default" href="https://wa.me/521"'.$reg->telefono.'">'.$reg->telefono. '</a>',
 				"5"=>$reg->email
 				);
 		}
 		$results = array(
